<?php $__env->startSection('title', 'Dashboard Admin - UP Cireng'); ?>
...
<?php $__env->startPush('head'); ?>
    ...
    <?php if(config('broadcasting.default') !== 'null'): ?>
        <script src="//<?php echo e(request()->getHost()); ?>:8080/socket.io/socket.io.js"></script>
        <script src="https://cdn.jsdelivr.net/npm/laravel-echo@1.15.1/dist/echo.iife.js"></script>
        <script>
            window.Echo = new Echo({
                broadcaster: 'reverb',
                key: '<?php echo e(env('REVERB_APP_KEY')); ?>',
                wsHost: '<?php echo e(env('REVERB_HOST', request()->getHost())); ?>',
                wsPort: <?php echo e(env('REVERB_PORT', 8080)); ?>,
                wssPort: <?php echo e(env('REVERB_PORT', 443)); ?>,
                forceTLS: <?php echo e(env('APP_ENV') === 'production' ? 'true' : 'false'); ?>,
...
<?php $__env->startPush('scripts'); ?>
    ...
    <script src="<?php echo e(asset('js/realtime-notifications.js')); ?>"></script>
<?php $__env->stopPush(); ?>
    ...
    <main class="px-4 py-6 sm:px-5 sm:py-8 lg:px-8 xl:px-10">
        ...
        <section class="mb-10 sm:mb-12 lg:mb-16 rounded-2xl sm:rounded-3xl bg-gradient-to-br from-brand-50 via-white to-brand-50/50 p-5 sm:p-8 lg:p-12 shadow-lg border border-brand-100/50" data-animate="fade-in-up" data-delay="100">
            <div class="flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
                ...
                <div class="flex flex-wrap gap-2 sm:gap-3 xl:justify-end">
                    ...
                    <a href="<?php echo e(route('admin.customers')); ?>" class="inline-flex items-center justify-center gap-2 rounded-xl sm:rounded-2xl border-2 border-slate-200 bg-white px-4 sm:px-6 py-2.5 sm:py-3 text-sm font-bold text-slate-800 transition-all duration-300 hover:border-brand-400 hover:shadow-xl hover:-translate-y-1 hover:bg-brand-50 min-h-11">
                        ...
                        <span class="inline-flex items-center justify-center rounded-full bg-brand-100 px-2 py-0.5 text-xs font-bold text-brand-700 ml-1"><?php echo e($totalCustomers ?? 0); ?></span>
                    </a>

                    <a href="<?php echo e(route('admin.products.index')); ?>" class="inline-flex items-center justify-center gap-2 rounded-xl sm:rounded-2xl border-2 border-slate-200 bg-white px-4 sm:px-6 py-2.5 sm:py-3 text-sm font-bold text-slate-800 transition-all duration-300 hover:border-brand-400 hover:shadow-xl hover:-translate-y-1 hover:bg-brand-50 min-h-11">
        ...
        <section class="grid gap-4 sm:gap-5 mb-10 sm:mb-12 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6" data-animate="fade-in-up" data-delay="200">
            ...
            <div class="group xl:col-span-2 rounded-2xl sm:rounded-3xl bg-white p-5 sm:p-6 shadow-panel border border-slate-100 hover:shadow-2xl hover:-translate-y-2 transition-all duration-500">
                ...
                <p class="text-2xl sm:text-3xl font-black text-ink-950 leading-tight break-words">Rp <?php echo e(number_format($totalRevenue ?? 0, 0, ',', '.')); ?></p>
            </div>
            ...
            <div class="group rounded-2xl sm:rounded-3xl bg-white p-5 sm:p-6 shadow-panel border border-slate-100 hover:shadow-2xl hover:-translate-y-2 transition-all duration-500">
                ...
                <p id="metricTotalOrders" class="text-2xl sm:text-3xl font-black text-ink-950"><?php echo e($totalOrders ?? 0); ?></p>
            </div>
            ...
            <div class="group rounded-2xl sm:rounded-3xl bg-white p-5 sm:p-6 shadow-panel border border-slate-100 hover:shadow-2xl hover:-translate-y-2 transition-all duration-500">
                ...
                <p id="metricPendingOrders" class="text-2xl sm:text-3xl font-black text-amber-700"><?php echo e($pendingOrders ?? 0); ?></p>
            ...
            <div class="group rounded-2xl sm:rounded-3xl bg-white p-5 sm:p-6 shadow-panel border border-slate-100 hover:shadow-2xl hover:-translate-y-2 transition-all duration-500">
                ...
                <p class="text-2xl sm:text-3xl font-black text-emerald-700"><?php echo e($completedOrders ?? 0); ?></p>
            </div>
            ...
            <div class="group rounded-2xl sm:rounded-3xl bg-white p-5 sm:p-6 shadow-panel border border-slate-100 hover:shadow-2xl hover:-translate-y-2 transition-all duration-500">
                ...
                <p class="text-2xl sm:text-3xl font-black text-sky-700"><?php echo e($totalCustomers ?? 0); ?></p>
                <?php if(($newCustomers ?? 0) > 0): ?>
                    <p class="text-xs text-emerald-600 font-bold mt-2">+<?php echo e($newCustomers); ?> hari ini</p>
                <?php endif; ?>
            </div>
        </section>
            ...
            <div class="space-y-6 lg:space-y-8">
                ...
                <div class="rounded-2xl sm:rounded-3xl bg-white p-5 sm:p-6 lg:p-8 shadow-panel border border-slate-100">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
                        ...
                        <a href="<?php echo e(route('admin.products.index')); ?>" class="text-brand-500 font-bold hover:text-brand-600 text-sm whitespace-nowrap">Lihat Semua →</a>
                    </div>
                        ...
                        <?php $__empty_1 = true; $__currentLoopData = $topProducts ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <div class="flex items-center justify-between gap-3 p-4 rounded-2xl bg-gradient-to-r from-slate-50 to-transparent hover:from-brand-50 transition-all border border-slate-100 hover:border-brand-200">
                                <div class="flex items-center gap-3 min-w-0">
                                    ...
                                    <div class="min-w-0">
                                        <p class="font-bold text-slate-900 text-sm truncate"><?php echo e($product['name'] ?? 'N/A'); ?></p>
                                        <p class="text-xs text-slate-500"><?php echo e($product['quantity'] ?? 0); ?> terjual</p>
                                    </div>
                                </div>
                                <p class="font-black text-ink-950 text-sm ml-2 flex-shrink-0">Rp <?php echo e(number_format($product['revenue'] ?? 0, 0, ',', '.')); ?></p>
                            </div>
                ...
                <div class="rounded-2xl sm:rounded-3xl bg-gradient-to-br from-brand-500 via-brand-600 to-brand-700 p-5 sm:p-6 lg:p-8 shadow-panel">
                    ...
                    <div class="space-y-3">
                        <a href="<?php echo e(route('admin.customers')); ?>" class="block w-full rounded-2xl bg-white/20 hover:bg-white/30 px-5 py-3.5 text-sm font-bold text-white transition-all border border-white/20 hover:-translate-y-0.5">
                        ...
                        <a href="<?php echo e(route('admin.products.create')); ?>" class="block w-full rounded-2xl bg-white/95 hover:bg-white px-5 py-3.5 text-center font-bold text-brand-700 text-sm transition-all hover:shadow-xl hover:-translate-y-1">
                            <span class="flex items-center justify-center gap-2">
                        ...
                        <form action="<?php echo e(route('admin.recap.send')); ?>" method="POST" class="block w-full">
                            <?php echo csrf_field(); ?>
        ...
        <section class="mt-14 sm:mt-16 lg:mt-20" data-animate="fade-in-up" data-delay="400">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-8 sm:mb-10">
                ...
                <a href="<?php echo e(route('admin.orders')); ?>" class="inline-flex items-center justify-center gap-2 rounded-2xl border-2 border-slate-200 bg-white px-5 sm:px-6 py-3 sm:py-4 text-sm sm:text-base font-bold text-slate-800 hover:border-brand-400 hover:shadow-2xl hover:-translate-y-1 transition-all">
                            ...
                            <div class="relative z-10 flex-1 flex flex-col">
                                <div class="flex items-start justify-between gap-3 mb-4">
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <p class="text-lg sm:text-2xl font-black text-brand-600 truncate"><?php echo e($order->reference ?? 'N/A'); ?></p>
                                    </div>
                                    <span class="px-2.5 py-1 rounded-xl text-[10px] sm:text-xs font-bold <?php echo e($order->status_color ?? 'text-slate-600'); ?> whitespace-nowrap flex-shrink-0">
                                        <?php echo e($order->status_label ?? 'Unknown'); ?>
                                ...
                                <div class="space-y-3 mb-5 flex-1">
                                    <div>
                                        ...
                                        <p class="font-black text-slate-900 text-sm truncate"><?php echo e($order->customer_name ?? 'N/A'); ?></p>
                                        <p class="text-xs text-slate-500 truncate"><?php echo e($order->customer_phone ?? '-'); ?></p>
                                    </div>

                                    <div class="grid grid-cols-2 gap-2">
                                        <div class="rounded-xl bg-slate-50 p-3">
                                            <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Item</p>
                                            <p class="text-lg font-black text-ink-950 mt-1"><?php echo e(is_array($order->items_summary ?? null) ? count($order->items_summary) : 0); ?></p>
                                        </div>
                                        <div class="rounded-xl bg-slate-50 p-3">
                                            <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Total</p>
                                            <p class="text-sm font-black text-ink-950 mt-1 truncate"><?php echo e(method_exists($order, 'formatPrice') ? $order->formatPrice() : 'Rp 0'); ?></p>
                                        </div>
                                    </div>
                                </div>
...
// ── ★ REALTIME POLLING ─────────────────────────────────────────────────────
(function () {
    const POLL_URL     = '<?php echo e(route("admin.api.realtime-orders")); ?>';
    const ORDERS_URL   = '<?php echo e(route("admin.orders")); ?>';
    const POLL_INTERVAL = 5000;

    let lastKnownId    = <?php echo e((int) ($recentOrders->first()?->id ?? 0)); ?>;
...
<?php echo $__env->make('layout.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\user\Desktop\UP Cireng\upcireng\resources\views\admin\dashboard.blade.php ENDPATH**/ ?>