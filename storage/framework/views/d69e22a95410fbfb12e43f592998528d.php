<?php $__env->startSection('title', 'Detail Pesanan - UP Cireng'); ?>
    ...
    <div class="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8">
        <div class="rounded-[2rem] bg-white p-8 shadow-xl shadow-brand-500/20">
            <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                <div>
                    <p class="text-sm font-semibold uppercase tracking-[0.3em] text-brand-500"><?php echo e($order->reference); ?></p>
                    <h1 class="mt-3 text-4xl font-black text-slate-950"><?php echo e($order->summary_title); ?></h1>
                    <p class="mt-3 text-sm text-slate-600">Dibuat pada <?php echo e($order->created_at->translatedFormat('d F Y, H:i')); ?></p>
                </div>
                <div class="flex flex-wrap gap-3">
                    ...
                    <a href="<?php echo e(route('orders.my')); ?>" class="rounded-full border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-brand-300 hover:text-brand-600">
                        ← Kembali
                    </a>
                </div>
            </div>
            ...
            <div class="mt-8 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
                <div class="space-y-6">
                        ...
                        <div class="mt-4 space-y-3">
                            <?php $__currentLoopData = $order->items_summary; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="flex items-center justify-between gap-4 rounded-2xl bg-white px-4 py-3 shadow-sm">
                                    <div>
                                        <p class="font-semibold text-slate-900"><?php echo e($item['product_name']); ?></p>
                                        <p class="text-sm text-slate-500">
                                            <?php if($item['variant']): ?><?php echo e($item['variant']); ?> · <?php endif; ?>
                                            <?php echo e($item['quantity']); ?> item
                                        </p>
                                    </div>
                                    <p class="text-sm font-bold text-slate-900">Rp <?php echo e(number_format($item['subtotal'], 0, ',', '.')); ?></p>
                                </div>
                    ...
                    <section class="rounded-[1.5rem] bg-slate-50 p-5">
                        ...
                        <div class="mt-4 grid gap-4 md:grid-cols-2">
                            <div>
                                <p class="text-sm font-semibold text-slate-900"><?php echo e($order->customer_name); ?></p>
                                <p class="text-sm text-slate-600"><?php echo e($order->customer_phone); ?></p>
                                <p class="text-sm text-slate-600"><?php echo e($order->customer_email); ?></p>
                            </div>
                            <div>
                                <p class="text-sm font-semibold text-slate-900">Alamat</p>
                                <p class="text-sm text-slate-600"><?php echo e($order->delivery_address); ?></p>
                            </div>
                        </div>
                        <?php if($order->notes): ?>
                ...
                <div class="space-y-6">
                    <section class="rounded-[1.5rem] bg-gradient-to-br from-ink-950 to-brand-700 p-5 text-white">
                        ...
                        <p class="mt-4 text-3xl font-black"><?php echo e($order->formatPrice()); ?></p>
                        <p class="mt-2 text-sm text-white/80"><?php echo e(ucwords(str_replace('_', ' ', $order->payment_method))); ?></p>
                        <?php if($order->payment_proof_url): ?>
                            <a href="<?php echo e($order->payment_proof_url); ?>" target="_blank" rel="noopener" class="mt-4 inline-flex rounded-full bg-white px-4 py-2 text-sm font-semibold text-ink-950 transition duration-300 hover:scale-105 hover:shadow-lg">
                    ...
                    <section class="rounded-[1.5rem] border border-slate-200 bg-white p-5">
                        ...
                        <div class="mt-4 flex items-center justify-between">
                            <p class="text-lg font-bold text-slate-950"><?php echo e($order->sync_status_label); ?></p>
                            <?php if($order->can_retry_sync): ?>
                                <form action="<?php echo e(route('order.retry-sync', $order)); ?>" method="POST" class="inline">
                                    <?php echo csrf_field(); ?>
                        ...
                        <?php if($order->sync_error_label): ?>
                            <div class="mt-3 rounded-lg bg-amber-50 border border-amber-200 px-3 py-2">
                                <p class="text-xs text-amber-700"><?php echo e($order->sync_error_label); ?></p>
                            </div>
                        <?php endif; ?>
                    </section>
...
<script>
// ── Data dari Laravel ──────────────────────────────────────────────────────
const WA_MESSAGE  = `<?php echo e(addslashes($whatsappMessage)); ?>`;
const WA_SEND_URL = '<?php echo e($whatsappUrl); ?>';
const WA_ADMIN_URL = '<?php echo e($adminWaUrl); ?>';
...
<?php echo $__env->make('layout.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\user\Desktop\UP Cireng\upcireng\resources\views/order/show.blade.php ENDPATH**/ ?>