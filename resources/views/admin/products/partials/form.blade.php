<div class="space-y-6 sm:space-y-8">
  ...
  <div class="grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-2">
    <div>
      ...
      <input
        ...
        class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-900 shadow-sm transition-all duration-300 focus:border-brand-400 focus:ring-4 focus:ring-brand-100/50 hover:border-slate-300 hover:shadow-md sm:px-5 sm:py-4 @error('name') border-red-300 ring-2 ring-red-200/50 @enderror"
    ...
    <div>
      ...
      <div class="relative">
        ...
        <input
          ...
          class="w-full rounded-xl border border-slate-200 pl-11 pr-4 py-3 text-sm font-semibold text-slate-900 shadow-sm transition-all duration-300 focus:border-brand-400 focus:ring-4 focus:ring-brand-100/50 hover:border-slate-300 hover:shadow-md sm:pl-12 sm:pr-5 sm:py-4 @error('price') border-red-300 ring-2 ring-red-200/50 @enderror"
          placeholder="25000"
          required
          data-price-format
  ...
  <div class="grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-2">
    <div>
      ...
      <select
        id="status"
        name="status"
        class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-900 shadow-sm transition-all duration-300 focus:border-brand-400 focus:ring-4 focus:ring-brand-100/50 hover:border-slate-300 hover:shadow-md sm:px-5 sm:py-4 @error('status') border-red-300 ring-2 ring-red-200/50 @enderror"
        required
      >
        <option value="active"   {{ old('status', $product?->status ?? 'active') == 'active'   ? 'selected' : '' }}>✅ Aktif</option>
        <option value="inactive" {{ old('status', $product?->status ?? '')        == 'inactive' ? 'selected' : '' }}>⏸️ Nonaktif</option>
      </select>
      @error('status')
    ...
    <div>
      ...
      <select
        id="stock_status"
        name="stock_status"
        class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-900 shadow-sm transition-all duration-300 focus:border-brand-400 focus:ring-4 focus:ring-brand-100/50 hover:border-slate-300 hover:shadow-md sm:px-5 sm:py-4 @error('stock_status') border-red-300 ring-2 ring-red-200/50 @enderror"
        required
      >
        <option value="available"    {{ old('stock_status', $product?->stock_status ?? 'available') == 'available'    ? 'selected' : '' }}>🟢 Tersedia</option>
  ...
  <div class="grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-2">
    <div>
      ...
      <select
        id="is_open"
        name="is_open"
        class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-900 shadow-sm transition-all duration-300 focus:border-brand-400 focus:ring-4 focus:ring-brand-100/50 hover:border-slate-300 hover:shadow-md sm:px-5 sm:py-4 @error('is_open') border-red-300 ring-2 ring-red-200/50 @enderror"
        required
      >
        <option value="1" {{ (string)old('is_open', $product?->is_open ?? 1) == '1' ? 'selected' : '' }}>🟢 Ya, terima order</option>
        <option value="0" {{ (string)old('is_open', $product?->is_open ?? 1) == '0' ? 'selected' : '' }}>🔴 Tutup sementara</option>
      </select>
      @error('is_open')
        <p class="mt-1.5 text-xs font-medium text-red-600">{{ $message }}</p>
      @enderror
  ...
  <div>
    ...
    <textarea
      ...
      name="variants_input"
      rows="3"
      class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-900 shadow-sm transition-all duration-300 focus:border-brand-400 focus:ring-4 focus:ring-brand-100/50 hover:border-slate-300 hover:shadow-md sm:px-5 sm:py-4 @error('variants_input') border-red-300 ring-2 ring-red-200/50 @enderror"
      placeholder="Pisahkan dengan koma: Original, Keju, Pedas Level 1, Jumbo"
    >{{ old('variants_input', $product ? implode("\n", $product->availableVariants()) : '') }}</textarea>
    @error('variants_input')
      <p class="mt-1.5 text-xs font-medium text-red-600">{{ $message }}</p>
    @enderror
    <div id="variants-preview" class="mt-3 flex flex-wrap gap-2">
      @if($product)
        @foreach($product->availableVariants() as $variant)
          <span class="rounded-full bg-brand-100 px-3 py-1.5 text-xs font-bold text-brand-700">{{ $variant }}</span>
        @endforeach
      @endif
    </div>
  ...
  <div>
    ...
    <textarea
      ...
      name="description"
      rows="4"
      class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-900 shadow-sm transition-all duration-300 focus:border-brand-400 focus:ring-4 focus:ring-brand-100/50 hover:border-slate-300 hover:shadow-md sm:px-5 sm:py-4 sm:rows-5 @error('description') border-red-300 ring-2 ring-red-200/50 @enderror"
