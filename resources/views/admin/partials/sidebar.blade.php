...
<aside id="sidebar"
    ...
    <div class="relative flex flex-col gap-6">
        ...
        <div class="rounded-[1.6rem] border border-white/10 bg-white/5 p-5">
            ...
            @if(!is_null($adminSidebarMetricValue))
                <p class="mt-3 text-3xl font-extrabold">{{ $adminSidebarMetricValue }}</p>
            @endif

            <p class="mt-2 text-sm leading-7 text-slate-300">
                @if($adminSidebarMetricLabel)
                    <span class="mb-1.5 block text-xs font-bold uppercase tracking-[0.22em] text-slate-400">
                        {{ $adminSidebarMetricLabel }}
                    </span>
                @endif
                {{ $adminSidebarBody }}
            </p>
        ...
        <nav class="space-y-1.5" aria-label="Admin navigation">
            ...
            @foreach($navLinks as $link)
                ...
                <a href="{{ route($link['route']) }}"
                   class="{{ $navItemClass }} {{ $isActive ? $activeNavItemClass : $idleNavItemClass }}">
                    <span>{{ $link['label'] }}</span>
                    <span class="text-xs {{ $isActive ? 'text-brand-200' : 'text-slate-400' }}">
                        {{ $link['sub'] }}
                    </span>
                </a>
            @endforeach
