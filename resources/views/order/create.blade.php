...
@section('title', 'Order Manual - UP Cireng')
...
<div class="bg-[linear-gradient(180deg,#fff7ed_0%,#f8fafc_100%)]">
    <div class="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
        <div class="rounded-[2rem] bg-white p-8 shadow-xl shadow-brand-500/20">
            ...
            @if(!$storeOpen)
                <div class="mt-6 rounded-[1.5rem] border border-rose-200 bg-rose-50 px-5 py-4 text-sm font-semibold text-rose-700 flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="mt-1">Buka kembali pukul {{ $hours['start'] }} WIB</p>
                    </div>
                </div>
            @endif
            ...
            <form action="{{ route('order.store') }}" method="POST" enctype="multipart/form-data" class="mt-8 space-y-5">
                @csrf
                <div class="grid gap-5 md:grid-cols-2">
                    <div>
                        <label class="mb-2 block text-sm font-semibold text-slate-700">Produk</label>
                        <select name="product_id" class="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none transition focus:border-brand-400 focus:ring-4 focus:ring-brand-100" {{ $storeOpen ? '' : 'disabled' }} required>
                            @foreach($products as $product)
                                <option value="{{ $product->id }}">{{ $product->name }} - {{ $product->formatPrice() }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="mb-2 block text-sm font-semibold text-slate-700">Varian</label>
                        <input type="text" name="variant" value="{{ old('variant') }}" class="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none transition focus:border-brand-400 focus:ring-4 focus:ring-brand-100" placeholder="Opsional" {{ $storeOpen ? '' : 'disabled' }}>
                    </div>
                </div>
                ...
                <div class="grid gap-5 md:grid-cols-2">
                    <div>
                        ...
                        <input type="number" name="quantity" min="1" value="{{ old('quantity', 1) }}" class="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none transition focus:border-brand-400 focus:ring-4 focus:ring-brand-100" {{ $storeOpen ? '' : 'disabled' }} required>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="grid gap-3 grid-cols-2 sm:grid-cols-4">
                        <!-- COD Card -->
                        <button type="button" class="payment-method-btn group rounded-2xl border-2 border-slate-200 px-4 py-4 text-center transition-all duration-300 hover:border-brand-400 focus:outline-none focus:ring-2 focus:ring-brand-400 focus:ring-offset-2" data-method="cod" {{ $storeOpen ? '' : 'disabled' }}>
                        ...
                        <!-- Transfer Bank Card -->
                        <button type="button" class="payment-method-btn group rounded-2xl border-2 border-slate-200 px-4 py-4 text-center transition-all duration-300 hover:border-brand-400 focus:outline-none focus:ring-2 focus:ring-brand-400 focus:ring-offset-2" data-method="bank_transfer" {{ $storeOpen ? '' : 'disabled' }}>
                            <div class="text-2xl mb-2">🏦</div>
                        ...
                        <!-- E-Wallet Card -->
                        <button type="button" class="payment-method-btn group rounded-2xl border-2 border-slate-200 px-4 py-4 text-center transition-all duration-300 hover:border-brand-400 focus:outline-none focus:ring-2 focus:ring-brand-400 focus:ring-offset-2" data-method="ewallet" {{ $storeOpen ? '' : 'disabled' }}>
                            <div class="text-2xl mb-2">📱</div>
                        ...
                        <!-- QRIS Card -->
                        <button type="button" class="payment-method-btn group rounded-2xl border-2 border-slate-200 px-4 py-4 text-center transition-all duration-300 hover:border-brand-400 focus:outline-none focus:ring-2 focus:ring-brand-400 focus:ring-offset-2" data-method="qris" {{ $storeOpen ? '' : 'disabled' }}>
                            <div class="text-2xl mb-2">📲</div>
                    ...
                    <input type="hidden" name="payment_method" id="payment_method" value="{{ old('payment_method', 'cod') }}" required>
                </div>
                ...
                <div class="grid gap-5 md:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="customer_name" value="{{ old('customer_name', $user->name) }}" class="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none transition focus:border-brand-400 focus:ring-4 focus:ring-brand-100" {{ $storeOpen ? '' : 'disabled' }} required>
                    </div>
                    <div>
                        <label class="mb-2 block text-sm font-semibold text-slate-700">WhatsApp</label>
                        <input type="text" name="customer_phone" value="{{ old('customer_phone', $user->phone) }}" class="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none transition focus:border-brand-400 focus:ring-4 focus:ring-brand-100" {{ $storeOpen ? '' : 'disabled' }} required>
                    </div>
                </div>

                <div>
                    <label class="mb-2 block text-sm font-semibold text-slate-700">Email</label>
                    <input type="email" name="customer_email" value="{{ old('customer_email', $user->email) }}" class="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none transition focus:border-brand-400 focus:ring-4 focus:ring-brand-100" {{ $storeOpen ? '' : 'disabled' }} required>
                </div>

                <div>
                    <label class="mb-2 block text-sm font-semibold text-slate-700">Alamat Pengiriman</label>
                    <textarea name="delivery_address" rows="4" class="w-full rounded-[1.5rem] border border-slate-200 px-4 py-3 outline-none transition focus:border-brand-400 focus:ring-4 focus:ring-brand-100" {{ $storeOpen ? '' : 'disabled' }} required>{{ old('delivery_address') }}</textarea>
                </div>
                ...
                <div id="payment-proof-section" class="hidden space-y-3 rounded-2xl bg-amber-50 border border-amber-200 p-5 animate-fade-in">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex-1">
                            ...
                            <input type="file" name="payment_proof" accept="image/*" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm transition focus:border-brand-400 focus:ring-4 focus:ring-brand-100 @error('payment_proof') border-rose-500 @enderror" {{ $storeOpen ? '' : 'disabled' }}>
                            @error('payment_proof')
                ...
                <div>
                    ...
                    <textarea name="notes" rows="3" class="w-full rounded-[1.5rem] border border-slate-200 px-4 py-3 outline-none transition focus:border-brand-400 focus:ring-4 focus:ring-brand-100" {{ $storeOpen ? '' : 'disabled' }}>{{ old('notes') }}</textarea>
                </div>

                <button type="submit" class="w-full rounded-2xl px-5 py-3 text-sm font-bold transition {{ $storeOpen ? 'bg-gradient-to-r from-ink-950 to-brand-600 text-white hover:shadow-lg hover:scale-105' : 'cursor-not-allowed bg-slate-300 text-slate-500' }}" {{ $storeOpen ? '' : 'disabled' }}>
                    {{ $storeOpen ? '✓ Kirim Pesanan' : 'Toko Tutup' }}
                </button>
            </form>
        </div>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const selectedMethod = "{{ old('payment_method', 'cod') }}";
